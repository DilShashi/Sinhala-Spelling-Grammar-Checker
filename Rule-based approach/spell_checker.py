...
def matches_custom_rules(word, correct_words):
    ...
    rules = [
        ...
        lambda w: w[:-1] in correct_words if w.endswith("\u0DB6") else False,  # Ends with "ම"
        lambda w: w[:-2] in correct_words if w.endswith("\u0D9A\u0DA7") else False,  # Ends with "කට"
        lambda w: w[:-1] in correct_words if w.endswith("\u0DB2\u0DCA") else False,  # Ends with "ත්"
...
def correct_sinhala_text(paragraph):
    ...
    for sentence in sentences:
        ...
        # Skip correction for the first word (subject)
        for i, word in enumerate(tokens):
            if i == 0:  # First word (subject) is kept as is
                continue
